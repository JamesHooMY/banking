...
mysql -uroot -proot_password -e "SET GLOBAL super_read_only = OFF;"

# Configure the slave to replicate from the master
mysql -uroot -proot_password -e "CHANGE MASTER TO MASTER_HOST='mysql-master', MASTER_USER='replica', MASTER_PASSWORD='replica_password', MASTER_LOG_FILE='$MASTER_LOG_FILE', MASTER_LOG_POS=$MASTER_LOG_POS;"
mysql -uroot -proot_password -e "START SLAVE;"

# Re-enable super_read_only mode
mysql -uroot -proot_password -e "SET GLOBAL super_read_only = ON;"
